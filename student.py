...
class AdminPage:
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        conn = psycopg2.connect(database='postgres', user='postgres', password='0420lhzx', host='localhost', port='5432') # 打开数据库连接
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        self.age = []
        # 打开数据库连接
        conn = psycopg2.connect(database='postgres', user='postgres', password='0420lhzx', host='localhost', port='5432')
        cursor = conn.cursor()  # 使用cursor()方法获取操作游标
